...
The \texttt{print} function prints things to the screen.
		\begin{lstlisting}[language=Python]
a = 6 + 3
print(a)
...
print(numberlist)	\end{lstlisting}
		...
		The \texttt{range} function generates lists that are integer sequences between two specified numbers.
\begin{lstlisting}[language=Python]
my_list = range(3, 6)
print(my_list)	\end{lstlisting}
		\begin{verbatim}> [3, 4, 5]\end{verbatim}
...
print("Two more than your number is: ", user_input_number+2)\end{lstlisting}
		\begin{verbatim}> Two more than your number is: 42.0\end{verbatim}
...
In this example lines 1 and 2 define the function. The code in the function does not run until it is called in line 4. When the program runs it first saves the function \texttt{hungarian\_phrasebook} to memory and then begins executing the rest of the code on line 3. 
...
main()\end{lstlisting}
			\begin{verbatim}> 6
				...
				Subroutines don't just have to print values. They can do some processing and then return the value to the main loop using the \texttt{return} function.
...
Note the use of the \texttt{len} function inside \texttt{range}. \texttt{len} returns the number of elements in a list, in this case in combination with \texttt{range} it is an easy way to iterate through the whole list. There is an easier way though, as we saw earlier, Python treats lists as iterables which means that a for loop can refer to them directly,
...
\begin{task}Generate a large Python list of data and using a loop take the sqaure root of each element in the list. Now compare the speed when looping through a NumPy array. Finally use the np.sqrt function (Google for how to use it) to vectorise the operation. How much faster is this?\end{task} 
...
Python has a particular style which is not particularly important for these workshops but is worth discussion in case you want to study the language further. The standard coding style is set out in a series of documents called the Python Enhancement Proposals. Some of these documents are very serious and boring proposals while others are reccomendations in the form of poems. In summary, the principle of good Python is readability. The code that you write should be as understandable as possible. Code that follows this principle is called ``Pythonic''. There are certain common Pythonic ideoms one of which is seen when handling files. Instead of explicitly opening and closing a file we can use a while loop to do it implicitly
		\begin{lstlisting}[language=Python]
...
At the end of the day it is up to you. While there are some commonly adopted standards, you can do whatever you think makes your code the most readable.
...
Note again that file operations are done with strings so numbers have to be excplicitly converted before writing to the file. But what is the \texttt{``\\n"} at the end? This is the special character for a new line. This ensures that each of our numbers will be printed to a new line.
...
Since \texttt{loadtxt} reads the data into a NumPy array it also accepts only numbers as input. If you need to read in a mixture of text and numbers this is probably better done with a Python list and a basic file read loop. If you have numerical data with a more complex layout such as missing values then \texttt{numpy.genfromtxt} is a more complex but flexible file reading function.
	...
	\subsection{NumPy savetxt}
		In the same way as before, if you just havbe a simple numpy array of data, you can quickly save to a text file with NumPy savetxt,
		\begin{lstlisting}[language=Python]
...
\begin{task}Using the input from the previous exercise, read in the data using loadtxt, multiply by 4 and print to a file using numysavetxt. Check the savetxt documentation for how to save a title line before the data.\end{task}

\section{Matplotlib}
	\subsection{Histograms}
		There is a very powerful package called matplotlib that provides functions for plotting graphs. Matplotlib is used across academia for producing publication quality figures. At first it might seem like a lot of effort to learn an entirely new script based method for plotting compared to sommething like Excel but it is worth it. Matplotlib graphs look much better than Excel ones, they are much more customisable and more powerful. In addition, once you have a nice set of temlates set up you can use them all through your academic career, slightly modifying them to suit whatever data you have.
		\begin{lstlisting}[language=Python]
