...
print ("Your random number is: ", random())\end{lstlisting}
		...
		\begin{task}Program a game of higher or lower. Generate a random integer between 1 and 100. Allow the user to inout a guess and tell them if the guess is too high or too low until they guess correctly.\end{task}		
...
\begin{task}Generate a large Python list of data and using a loop take the sqaure root of each element in the list. How does looping through a NumPy array compare speedwise? Finally use the np.sqrt function (Google for how to use it) to vectorise the operation. How much faster is this?\end{task} 
