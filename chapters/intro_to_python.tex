...
print(numberlist[0:3])\end{lstlisting}
		...
		The append command will add things onto the end of a list. The append command is a method of the list object so acts on the list directly and the result does not need to be assigned to a variable as for a math operation like $\mathrm{a} = 5 + 3$.
		\begin{lstlisting}[language=Python]
numberlist = [1, 2, 5]
numberlist.append(8)
print(numberlist)	\end{lstlisting}
